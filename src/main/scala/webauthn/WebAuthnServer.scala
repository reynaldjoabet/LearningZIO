...
import webauthn.config.WebAuthnConfig
import webauthn.core.{UserService, WebAuthnServiceImpl}
...
object WebAuthnServer extends ZIOAppDefault {
  ...
  override val run = {
    val program = for {
      config  <- ZIO.config[WebAuthnConfig](WebAuthnConfig.config)
  ...
  private def printBanner(config: WebAuthnConfig): ZIO[Any, IOException, Unit] =
    for {
